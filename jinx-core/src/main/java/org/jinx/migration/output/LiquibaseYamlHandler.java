...
public class LiquibaseYamlHandler implements OutputHandler{
    ...
    public void handle(DiffResult diff, SchemaModel oldSchema, SchemaModel newSchema, DialectBundle dialect, Path outputDir) throws IOException {
        handle(diff, oldSchema, newSchema, dialect, outputDir, null);
    }

    public void handle(DiffResult diff, SchemaModel oldSchema, SchemaModel newSchema, DialectBundle dialect, Path outputDir, Naming naming) throws IOException {
        handle(diff, oldSchema, newSchema, dialect, outputDir, naming, null);
    }

    public void handle(DiffResult diff, SchemaModel oldSchema, SchemaModel newSchema, DialectBundle dialect, Path outputDir, Naming naming, MigrationInfo migrationInfo) throws IOException {
        DatabaseChangeLog yaml = new LiquibaseYamlGenerator().generate(diff, oldSchema, newSchema, dialect, naming, migrationInfo);
