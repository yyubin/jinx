...
public class RelationshipModel {
    ...
    @Builder.Default
    private boolean noConstraint = false; // Added for @ForeignKey(NO_CONSTRAINT)

    @Builder.Default
    private List<CascadeType> cascadeTypes = new ArrayList<>(); // Added for cascade

    @Builder.Default
    private boolean orphanRemoval = false; // Added for orphanRemoval

    @Builder.Default
    private FetchType fetchType = FetchType.LAZY; // Added for fetch

    private String sourceAttributeName; // 소스 엔티티의 속성명
    
    // @MapsId 매핑 정보 (지연 처리 패스에서 설정)
    @Builder.Default
    private Map<String, String> mapsIdBindings = new HashMap<>(); // FK컬럼 → 소유 PK컬럼
