...
* `@Access(PROPERTY)` 및 혼합 접근 전략 지원
* JOINED 상속의 `@PrimaryKeyJoinColumn(s)` / `@SecondaryTable.pkJoinColumns` 복합키 처리
* 복수 `@JoinColumns`/`inverseJoinColumns` 전부 반영 및 검증
* `@JoinColumn.foreignKey`, `@JoinTable.foreignKey` 이름 반영
* SINGLE\_TABLE 기본 Discriminator 컬럼/타입/길이 적용
* `@SecondaryTable` 의 `indexes`/`uniqueConstraints` 처리
* FK 컬럼 생성시 `nullable/unique/precision/scale/columnDefinition` 반영
* `@CollectionTable(indexes/uniqueConstraints)`, `@MapKeyJoinColumn(s)` 등 Map 확장
* `@OrderBy` 메타데이터 보존(선택: 인덱스 힌트)
* `@MapsId("attr")` 명시적 매핑
* Dialect 기반 `AUTO` 전략 결정
* Bean Validation → DDL 힌트(선택)
* 단방향 `@OneToMany` FK 위치/전략 재검토
* `columnDefinition` 과 defaultValue 의미 분리 (`sqlTypeOverride` 등)
